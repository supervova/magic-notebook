...
The `/styles` directory contains all base styles, design tokens, resets, global settings, and utilities that form the foundation of the project's visual language.
...
- **Modern CSS**: The framework leverages modern CSS capabilities such as logical properties, `@scope`, `:has()`, and nesting.
...
- **Scale**: Font sizes are based on a base size (`--font-size-base: 17px`) and a scaling factor (`--scale`), which changes at different breakpoints. For marketing pages on large screens, we can increase the font size across all levels by adjusting only the base size.

```css
.page-landing { --font-size-base: 20px; }
```
